...
function the_dp_logo(){
  ...
  if($logo){
	...
	echo '<h1 id="logo" style="top:' . $option['logotop'] . 'px; left:' . $option['logoleft'] . 'px;"><a href=" ' . $url . '/" title="' . $title . '" data-label="' . $title . '"><img class="h_logo" src="' . $logo['url'] . '?' . time() . '" alt="' . $title . '" title="' . $title . '" /></a></h1>' . "\n";
	echo "</div>\n";
  } else {
	echo "<div id='logo_text'>\n";
	echo '<h1 id="logo"><a href="' . $url . '/">' . $title . "</a></h1>\n";
	echo '<p id="description">'. $tagline ."</p>\n";
	echo "</div>\n";
  }
}
...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		for($i = 1; $i <= 8; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_bg_image'.$i)){
				$file = basename($options['bg_image'.$i]);
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_second_leadcopy_image'.$i)){
				$file = basename($options['second_leadcopy_image']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['second_leadcopy_image'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image'.$i)){
				$file = basename($options['slider_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_third_banner_image'.$i)){
				$file = basename($options['third_banner_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['third_banner_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_fifth_banner_image'.$i)){
				$file = basename($options['fifth_banner_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['fifth_banner_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 2; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_single_ad_image'.$i)){
				$file = basename($options['single_ad_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['single_ad_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_mobile_ad_image'.$i)){
				$file = basename($options['mobile_ad_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['mobile_ad_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
