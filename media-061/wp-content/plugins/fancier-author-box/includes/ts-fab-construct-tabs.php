		...
		<div class="ts-fab-avatar">';
			...
			$ts_fab_bio .= '<div class="ts-fab-social-links">';
				...
				if ( get_user_meta( $author->ID, 'ts_fab_twitter', true ) ) {
					...
					$ts_fab_bio .= '<a href="' . esc_attr( $ts_fab_twitter_url ) . '" title="Twitter" rel="nofollow" target="_blank"><img src="' . plugins_url( 'images/twitter.png', dirname(__FILE__) ) . '" width="24" height="24" alt="' . __( 'My Twitter profile', 'ts-fab' ) . '" /></a>';
				}
				...
				if ( get_user_meta( $author->ID, 'ts_fab_facebook', true ) ) {
					...
					$ts_fab_bio .= '<a href="' . esc_attr( $ts_fab_facebook_url ) . '" title="Facebook" rel="nofollow" target="_blank"><img src="' . plugins_url( 'images/facebook.png', dirname(__FILE__) ) . '" width="24" height="24" alt="' . __( 'My Facebook profile', 'ts-fab' ) . '" /></a>';
				}
				...
				if ( get_user_meta( $author->ID, 'ts_fab_googleplus', true ) ) {
					...
					$ts_fab_bio .= '<a href="' . esc_attr( $ts_fab_googleplus_url ) . '" title="Google+" target="_blank"><img src="' . plugins_url( 'images/googleplus.png', dirname(__FILE__) ) . '" width="24" height="24" alt="' . __( 'My Google+ profile', 'ts-fab' ) . '" /></a>';
				}
			...
			if( get_user_meta( $author->ID, 'ts_fab_position', true) ) {
				...
				if( get_user_meta( $author->ID, 'ts_fab_company', true) ) {
					if( get_user_meta( $author->ID, 'ts_fab_company_url', true) ) {
						$ts_fab_bio .= ' ' . __( 'at', 'ts-fab' ) . ' <a href="' . esc_url( get_user_meta( $author->ID, 'ts_fab_company_url', true) ) . '" rel="nofollow" target="_blank">';
...
function ts_fab_show_latest_posts( $context = '', $authorid = '' ) {
	...
	<div class="ts-fab-tab" id="ts-fab-latest-posts-' . $context . '">
		...
		<div class="ts-fab-text">
			<div class="ts-fab-header">
				<h4>' . __( 'Latest posts by ', 'ts-fab' ) . $author->display_name . ' <span class="latest-see-all">(<a href="' . get_author_posts_url( $author->ID ) . '" rel="nofollow" target="_blank">' . __( 'see all', 'ts-fab' ) . '</a>)</span></h4>
			</div>
